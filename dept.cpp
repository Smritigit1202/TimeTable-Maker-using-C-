...
{
    cout << "Dept. Name : " << p.name << endl;
    out << "Course type : ";
    out << "For day maximum " << p.max_class[0] << " tuts, ";
    out << p.max_class[1] << " labs+ " << p.max_class[2] << " lectures allowed. \n";
    out << "for 1 tut there are " << p.class_dur[0] << " hour(s) " << endl;
    out << "for 1 lab there are " << p.class_dur[1] << " hour(s) " << endl;
    out << "for 1 lecture there are " << p.class_dur[2] << " hour(s) " << endl;
    out << "A break of " << p.Break << " hours." << endl;
    string course_type[] = {"DEPT CORE", "DEPT ELECTIVE", "HSS ELECTIVE", "MINOR"};
    for (int i = 0; i < 4; i++)
    {
        out << course_type[i] << "\n";
        p.d[i].print(out);
...
bool dept::checkData()
{
    ...
    if (m_lec > 6)
    {
        cout << " max 6 lectures per day. " << endl;
        flag = false;
    }
    if (m_tut > 2)
    {
        cout << " max 2 tutorials per day. " << endl;
        flag = false;
    }
    if (m_lab > 6)
    {
        cout << " max 1 lab per day. " << endl;
        flag = false;
    }
    if (t_lec > 5 * m_lec)
    {
        cout << "Insufficient time." << endl;
        flag = false;
    }
    if (t_lab > 5 * m_lab)
    {
